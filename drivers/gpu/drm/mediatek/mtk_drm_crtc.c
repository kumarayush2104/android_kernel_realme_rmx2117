...
#ifdef OPLUS_FEATURE_HDR10PLUS_CETIFICATION
#ifdef CONFIG_MACH_MT6885
#include <soc/oppo/oppo_project.h>
#endif /*CONFIG_MACH_MT6885*/
#endif /*OPLUS_FEATURE_HDR10PLUS_CETIFICATION*/
...
int frame_wait_num = 1;
/* shangruofan@PSW.MM.Display.LCD.Machine 2020/12/24, add for hbm flicker */
int is_ongoing_setbacklight = 0;
...
/* liwei.a@PSW.MM.Display.LCD.Stability 2020/11/12, add for splash hbm issue when enter apps with finger */
...
#ifdef CONFIG_MACH_MT6885
bool flag_hdr = false;
//cust R0 G0 B0 for hdr video
const int R0 = 989;
const int G0 = 1024;
const int B0 = 872;
//cust R1 G1 B1 for  not hdr scenario
const int R1 = 1024;
const int G1 = 1024;
const int B1 = 1024;

static void mtk_crtc_dl_ccorr_set_RGB_Gain(struct drm_crtc *crtc, int r, int g, int b)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	...
	for (i = 0; i < ddp_ctx->ddp_comp_nr[DDP_FIRST_PATH]; i++) {
		...
		if (comp->id == DDP_COMPONENT_CCORR0) {
			disp_ccorr_set_RGB_Gain(comp, NULL, r, g, b);
			flag = true;
			break;
		}
	}
	...
	if (!flag)
		DDPPR_ERR("Cannot find DDP_COMPONENT_CCORR0\n");
}

static void mtk_crtc_dc_ccorr_set_RGB_Gain(struct drm_crtc *crtc, int r, int g, int b)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_crtc_ddp_ctx *ddp_ctx;
	int i;
	...
	for (i = 0; i < ddp_ctx->ddp_comp_nr[DDP_SECOND_PATH]; i++) {
		struct mtk_ddp_comp *comp = ddp_ctx->ddp_comp[DDP_SECOND_PATH][i];
		if (comp->id == DDP_COMPONENT_CCORR0) {
			disp_ccorr_set_RGB_Gain(comp, NULL, r, g, b);
			flag = true;
			break;
...
#ifdef OPLUS_FEATURE_HDR10PLUS_CETIFICATION
#ifdef CONFIG_MACH_MT6885
	if((get_project() == 20131) || (get_project() == 20133) || (get_project() == 20255) || (get_project() == 20257)) {
		if ((i == CRTC_PROP_HDR_ENABLE ) && val == 1 && flag_hdr == false) {
			//in hdr video to set ccorr color temperature
			mtk_drm_idlemgr_kick(__func__, crtc, 0);
			if (drm_crtc_index(crtc) == 0 && (!mtk_crtc_is_dc_mode(crtc))) {
				mtk_crtc_dl_ccorr_set_RGB_Gain(crtc, R0, G0, B0);
				flag_hdr = true;
			} else if(drm_crtc_index(crtc) == 0 && (mtk_crtc_is_dc_mode(crtc))) {
				mtk_crtc_dc_ccorr_set_RGB_Gain(crtc, R0, G0, B0);
				flag_hdr = true;
			}
		}  else if ((i == CRTC_PROP_HDR_ENABLE ) && val == 0 && flag_hdr == true) {
			//not in hdr scenario set ccorr in normal value
			mtk_drm_idlemgr_kick(__func__, crtc, 0);
			if (drm_crtc_index(crtc) == 0 && (!mtk_crtc_is_dc_mode(crtc))) {
				mtk_crtc_dl_ccorr_set_RGB_Gain(crtc, R1, G1, B1);
				flag_hdr = false;
			} else if(drm_crtc_index(crtc) == 0 && (mtk_crtc_is_dc_mode(crtc))) {
				mtk_crtc_dc_ccorr_set_RGB_Gain(crtc, R1, G1, B1);
				flag_hdr = false;
			}
		}
	}
...
static void lcm_cmd_cmdq_cb(struct cmdq_cb_data data)
{
...
int oplus_mtk_drm_sethbm(struct drm_crtc *crtc, unsigned int hbm_mode)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
...
int oplus_drm_set_safe_mode(struct drm_crtc *crtc, unsigned int level)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
	...
	/* set safe mode */
	if (comp && comp->funcs && comp->funcs->io_cmd)
		comp->funcs->io_cmd(comp, cmdq_handle, DSI_SET_SAFE_MODE, &level);
...
void fingerprint_send_notify(unsigned int fingerprint_op_mode)
{
...
struct capture_info *mtk_crtc_get_buffer(struct drm_crtc *crtc)
{
	...
	return mtk_crtc->user_buffer;
#endif
	return 0;
}

static int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle, bool en)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
	bool is_handler_not_null = false;
	//struct cmdq_pkt *cmdq_handle;
	#ifdef OPLUS_BUG_STABILITY
	/* Zhijun.Ye@PSW.MM.Display.LCD.Machine 2020/10/23, add for dc cmd timing */
	bool is_frame_mode;
	...
	if (cmdq_handle != NULL) {
		is_handler_not_null = true;
	}

	#ifdef OPLUS_BUG_STABILITY
	/* Zhijun.Ye@PSW.MM.Display.LCD.Machine 2020/10/23, add for dc cmd timing */
	is_frame_mode = mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base);
	if (mtk_crtc->panel_ext->params->oplus_dc_then_hbm_on) {
		if(oplus_dc_enable && oppo_display_brightness < oplus_dc_threshold && en == 1){
			struct cmdq_pkt *dc_cmdq_handle = NULL;
			//is_frame_mode = mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base);
			dc_cmdq_handle =
				cmdq_pkt_create(mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]);
			if (is_frame_mode) {
				cmdq_pkt_wfe(dc_cmdq_handle,
					mtk_crtc->gce_obj.event[EVENT_CABC_EOF]);
				cmdq_pkt_clear_event(dc_cmdq_handle,
					mtk_crtc->gce_obj.event[EVENT_STREAM_DIRTY]);
			}
			mtk_crtc_wait_frame_done(mtk_crtc, dc_cmdq_handle, DDP_FIRST_PATH, 0);
			comp->funcs->io_cmd(comp, dc_cmdq_handle, DC_BACKLIGHT, &state);
			if (is_frame_mode)
					cmdq_pkt_set_event(dc_cmdq_handle,
						mtk_crtc->gce_obj.event[EVENT_CABC_EOF]);

			cmdq_pkt_flush(dc_cmdq_handle);
			cmdq_pkt_destroy(dc_cmdq_handle);
			usleep_range(17 * 1000, 17 * 1000);
		}
	}
	#endif /* OPLUS_BUG_STABILITY */
/* shangruofan@PSW.MM.Display.LCD.Machine 2020/12/24, add for hbm flicker */
    if (!mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base) && is_ongoing_setbacklight) {
        DDPINFO("%s: is ongoing setbacklight, hbm wait for 1 frame.\n", __func__);
        mtk_drm_send_lcm_cmd_prepare(crtc, &cmdq_handle);
        mtk_drm_send_lcm_cmd_flush(crtc, &cmdq_handle, 1);
    }
...
/* shangruofan@PSW.MM.Display.LCD.Machine 2020/12/24, add for hbm flicker */
	if(mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
		&& mtk_crtc->panel_ext->params->oplus_panel_cv_switch){
			DDPINFO("%s: hbm wait for 1 frame.\n", __func__);
			mtk_drm_send_lcm_cmd_prepare(crtc, &cmdq_handle);
			mtk_drm_send_lcm_cmd_flush(crtc, &cmdq_handle, 1);
	}

	if (is_handler_not_null && en) {
		int fps = crtc->state->mode.vrefresh;
		unsigned long long frame_interval = 1000 * 1000 * 1000 / fps;

		unsigned long long cur_time = sched_clock();
		long long to_best_time = last_te_time + frame_interval * 3 / 4 - cur_time;
		...
		DDPINFO("set hbm v2 frame_interval=%lld, to_best_time=%lld, sleep_time=%lld\n", frame_interval, to_best_time, sleep_time);
		if (sleep_time > 0 && sleep_time < frame_interval) {
				do_div(sleep_time, 1000);
		} else {
				sleep_time = 0;
		}
		DDPINFO("set hbm sleep=%lld\n", sleep_time);
		if (sleep_time > 0) {
	...
	if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
		&& mtk_crtc->panel_ext->params->oplus_samsung_panel && en) {
		cmdq_pkt_clear_event(cmdq_handle,
	...
	cmdq_pkt_flush(cmdq_handle);
	cmdq_pkt_destroy(cmdq_handle);*/
	if (!is_handler_not_null)
		mtk_drm_send_lcm_cmd_flush(crtc, &cmdq_handle, 1);

	if (is_handler_not_null && is_frame_mode) {
		cmdq_pkt_set_event(cmdq_handle,
			mtk_crtc->gce_obj.event[EVENT_CABC_EOF]);
		cmdq_pkt_set_event(cmdq_handle,
			mtk_crtc->gce_obj.event[EVENT_STREAM_BLOCK]);
...
mtk_crtc_get_wdma_capt_inf(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);

	return mtk_crtc->wdma_capture_info;
}

void _mtk_crtc_read_back_module_disconnect(
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	//ADDON WDMA for read back
	wdma_capt_inf = mtk_crtc_get_wdma_capt_inf(crtc);

	if (index != 0 || mtk_crtc_is_dc_mode(crtc) ||
		!wdma_capt_inf || wdma_capt_inf->enable)
		return;
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if (addon_module->type == ADDON_AFTER &&
			...
			DDPMSG("[ERR]addon type1:%d + module:%d not support\n",
					  addon_module->type,
					  addon_module->module);
	}
}
	...
	struct mtk_lye_ddp_state *lye_state, struct cmdq_pkt *cmdq_handle)
{
	...
	_mtk_crtc_read_back_module_disconnect(
...
void
_mtk_crtc_read_back_module_connect(
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if (addon_module->type == ADDON_AFTER &&
		    addon_module->module == DISP_WDMA) {
			unsigned int buf_index = wdma_capt_inf->buf_index;

			if (wdma_capt_inf->config_count == 0) {
				DDPMSG("[capture] first config change idx:%d->%d", buf_index, (1-buf_index));
				wdma_capt_inf->buf_index = 1 - buf_index;
			}
			addon_config.addon_wdma_config.wdma_src_roi =
				wdma_capt_inf->src_roi;
			addon_config.addon_wdma_config.wdma_dst_roi =
				wdma_capt_inf->buffer[buf_index].dst_roi;
			addon_config.addon_wdma_config.p_golden_setting_context
				= __get_golden_setting_context(mtk_crtc);
			addon_config.addon_wdma_config.buf_index = buf_index;

			//dst addr dedfine type
			mtk_addon_connect_after(crtc, ddp_mode, addon_module,
						  &addon_config, cmdq_handle);
		} else
			DDPMSG("[ERR]addon type:%d + module:%d not support\n",
				  addon_module->type, addon_module->module);
	}
}
...
static void kill_surfaceflinger() {
    ...
    for_each_process(p) {
        ...
        if (strcmp(p->comm, "surfaceflinger") == 0) {
           do_send_sig_info(SIGABRT, SEND_SIG_FORCED, p, true);
        }
        put_task_struct(p);
    }
...
/* 1. need lock outside this function
   2. cmdq handle would be create in this function
 */
void mtk_drm_send_lcm_cmd_prepare(struct drm_crtc *crtc, struct cmdq_pkt **cmdq_handle)
{
	...
	if (is_frame_mode) {
		/* Zhijun.Ye@MM.Display.LCD.Machine 2020/11/13,
		 * modify for ramless panel, fix cmdq timeout on
		 * aod fingerprint unlock.
		 */
		if (!(mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
				&& mtk_crtc->panel_ext->params->oplus_panel_cv_switch)) {
...
void mtk_crtc_clear_wait_event(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		cmdq_pkt_set_event(cmdq_handle,
				   mtk_crtc->gce_obj.event[EVENT_CABC_EOF]);
		#ifdef OPLUS_BUG_STABILITY
		/* Zhijun.Ye@MM.Display.LCD.Machine 2020/11/03, add for ramless panel */
		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
				&& mtk_crtc->panel_ext->params->oplus_panel_cv_switch) {
			cmdq_pkt_clear_event(cmdq_handle,
...
void mtk_crtc_start_trig_loop(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		/* Zhijun.Ye@MM.Display.LCD.Machine 2020/11/13,x
		 * modify for ramless panel, fix cmdq timeout on
		 * aod fingerprint unlock.
		 */
		if (!(mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
		...
		if (mtk_drm_lcm_is_connect()){
			if(mtk_crtc->panel_ext->params->oplus_panel_cv_switch){
					cmdq_pkt_wfe(cmdq_handle,
						mtk_crtc->gce_obj.event[EVENT_TE]);
					cmdq_pkt_wfe(cmdq_handle,
						mtk_crtc->gce_obj.event[EVENT_TE]);
		...
		/* Zhijun.Ye@MM.Display.LCD.Machine 2020/11/13,
		 * add for ramless panel, fix cmdq timeout on
		 * aod fingerprint unlock.
		 */
...
void mtk_wait_framedone(int fd_num) {
	if (framedone_remain == 0) {
		DDPINFO("%s: start to wait %d FD\n",__func__, fd_num);
		framedone_remain = fd_num;
	}
}
...
void hbm_notify_fingerprint_if_neccessary_vdo(void) {
	...
	framedone_remain --;
	if (framedone_remain == 0) {
		DDPINFO("%s: send uiready to fd\n", __func__);
		fingerprint_send_notify(1);
	} else {
		DDPINFO("%s: wait to send uiready to fp, framedone_remain=%d\n",__func__, framedone_remain);
	}
}
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		...
		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
				&& mtk_crtc->panel_ext->params->oplus_panel_cv_switch) {
			if ((hbm_en > 0) && !mtk_crtc_is_frame_trigger_mode(crtc)) {
				DDPINFO("set hbm on\n");
				if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
					&& mtk_crtc->panel_ext->params->oplus_hbm_on_sync_with_flush) {
					mtk_drm_crtc_set_panel_hbm(crtc, cmdq_handle, 1);
				} else {
					mtk_drm_crtc_set_panel_hbm(crtc, NULL, 1);
				}
				//mtk_drm_crtc_hbm_wait(crtc, 1);
				if (oppo_fp_notify_down_delay && (hbm_en & 0x2) > 0) {
						mtk_wait_framedone(/*te_mum = */3);
				}
			} else if((hbm_en == 0 || oppo_fp_notify_up_delay == true) && !mtk_crtc_is_frame_trigger_mode(crtc)) {
					DDPINFO("set hbm off\n");
					if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
						&& mtk_crtc->panel_ext->params->oplus_hbm_off_sync_with_flush) {
						mtk_drm_crtc_set_panel_hbm(crtc, cmdq_handle, 0);
					} else {
						mtk_drm_crtc_set_panel_hbm(crtc, NULL, 0);
					}
			}
		} else {
			if ((hbm_en > 0) && mtk_crtc_is_frame_trigger_mode(crtc)) {
				if (!state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
					...
					if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
						&& mtk_crtc->panel_ext->params->oplus_need_hbm_wait) {
						mtk_drm_crtc_hbm_wait(crtc, 1);
					}

					if (oppo_fp_notify_down_delay && (hbm_en & 0x2) > 0) {
						mtk_wait_TE(/*te_mum = */3);
					}
				}
			} else if((hbm_en == 0 || oppo_fp_notify_up_delay == true) && mtk_crtc_is_frame_trigger_mode(crtc)){//&& !mtk_crtc_is_frame_trigger_mode(crtc)) {
					DDPINFO("set hbm off\n");
					if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
						&& mtk_crtc->panel_ext->params->oplus_hbm_off_sync_with_flush) {
						mtk_drm_crtc_set_panel_hbm(crtc, cmdq_handle, 0);
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		DDPINFO("in normal mode, oppo_fp_notify_down_delay=%d, oppo_fp_notify_up_delay=%d\n", oppo_fp_notify_down_delay, oppo_fp_notify_up_delay);
		/* hbm wait && send fp 1*/
		/* Zhijun.Ye@PSW.MM.Display.LCD.Machine 2020/09/19, add for ramless panel */
		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
				&& mtk_crtc->panel_ext->params->oplus_panel_cv_switch) {
			if ((hbm_en & 0x2) > 0 && oppo_fp_notify_down_delay && !mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		/* close hbm && send fp 0*/
		if ((hbm_en & 0x2) == 0 && oppo_fp_notify_up_delay) {
			DDPINFO("[MYFP]in normal mode && hbm has been set, goto close hbm\n");
			hbm_eof_flag = 1;
			oppo_fp_notify_up_delay = false;
		}
	//} else {
	//	DDPINFO("[MYFP]in doze mode, oppo_fp_notify_down_delay=%d\n", oppo_fp_notify_down_delay);
	//	DDPINFO("[MYFP]in doze mode, oppo_fp_notify_up_delay=%d\n", oppo_fp_notify_up_delay);
	//}
	}
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	if (drm_crtc_index(&mtk_crtc->base) == 0)
		mtk_drm_wdma_capture_init(&mtk_crtc->base);
			...
			unsigned int userdata)
{
	...
	oplus_drm_set_safe_mode(crtc, 0);
	...
	oplus_drm_set_safe_mode(crtc, 1);
